    ...
    constructor(log, dAPI: Devolo, dDevices: Device[], storage, config) {
        ...
        self.dDoorDevice.events.on('onStateChanged', function(state: number) {
            ...
            if(state === self.Characteristic.ContactSensorState.CONTACT_NOT_DETECTED) {
                ...
                self.log.debug('%s (%s / %s) > onStateChanged > CurrentDoorState was %s, set to %s', (self.constructor as any).name, self.dDoorDevice.id, self.dDoorDevice.name, self.mappingDoorState(self.garageDoorLastTargetDoorState), self.mappingDoorState(self.Characteristic.CurrentDoorState.CLOSED));
