...
export class HBFakeGarageDoor implements HBIDevoloDevice {
    ...
    constructor(log, dAPI: Devolo, dDevices: Device[], storage, config) {
        ...
        self.dDoorDevice.events.on('onStateChanged', function(state: number) {
            ...
            if(state === self.Characteristic.ContactSensorState.CONTACT_NOT_DETECTED) {
                ...
                self.log.debug('%s (%s / %s) > onStateChanged > TargetDoorState was %s, set to %s', (self.constructor as any).name, self.dDoorDevice.id, self.dDoorDevice.name, self.convertDoorstateIDToText(self.garageDoorLastTargetDoorState), self.convertDoorstateIDToText(self.Characteristic.TargetDoorState.OPEN));
                ...
                self.log.debug('%s (%s / %s) > onStateChanged > TargetDoorState was %s, set to %s', (self.constructor as any).name, self.dDoorDevice.id, self.dDoorDevice.name, self.convertDoorstateIDToText(self.garageDoorLastTargetDoorState), self.convertDoorstateIDToText(self.Characteristic.TargetDoorState.CLOSED));
                ...
                self.log.debug('%s (%s / %s) > onStateChanged > CurrentDoorState was %s, set to %s', (self.constructor as any).name, self.dDoorDevice.id, self.dDoorDevice.name, self.convertDoorstateIDToText(self.garageDoorLastCurrentDoorState), self.convertDoorstateIDToText(self.Characteristic.CurrentDoorState.CLOSED));
    ...
    setTargetDoorState(value, callback) {
        ...
        */
        ...
        if(this.garageDoorLastCurrentDoorState === this.Characteristic.CurrentDoorState.CLOSED) {
            ...
            this.log.debug('%s (%s / %s) > setTargetDoorState > TargetDoorState was %s, set to %s', (this.constructor as any).name, this.dDoorDevice.id, this.dDoorDevice.name, this.convertDoorstateIDToText(this.garageDoorLastTargetDoorState), this.convertDoorstateIDToText(this.Characteristic.TargetDoorState.CLOSED));
            ...
            this.log.debug('%s (%s / %s) > setTargetDoorState > CurrentDoorState was %s, set to %s', (this.constructor as any).name, this.dDoorDevice.id, this.dDoorDevice.name, this.convertDoorstateIDToText(this.garageDoorLastCurrentDoorState), this.convertDoorstateIDToText(this.Characteristic.CurrentDoorState.CLOSING));
            ...
            this.log.debug('%s (%s / %s) > setTargetDoorState > CurrentDoorState was %s (OPENING/CLOSING), set to %s', (this.constructor as any).name, this.dDoorDevice.id, this.dDoorDevice.name, this.convertDoorstateIDToText(this.garageDoorLastCurrentDoorState), this.convertDoorstateIDToText(this.Characteristic.CurrentDoorState.STOPPED));
            ...
            if(this.garageDoorLastTargetDoorState === this.Characteristic.TargetDoorState.CLOSED) {
                ...
                this.log.debug('%s (%s / %s) > setTargetDoorState > TargetDoorState was %s, set to %s', (this.constructor as any).name, this.dDoorDevice.id, this.dDoorDevice.name, this.convertDoorstateIDToText(this.garageDoorLastTargetDoorState), this.convertDoorstateIDToText(this.Characteristic.TargetDoorState.OPEN));
                ...
                this.log.debug('%s (%s / %s) > setTargetDoorState > CurrentDoorState was %s, set to %s - OpeningTime is %s', (this.constructor as any).name, this.dDoorDevice.id, this.dDoorDevice.name, this.convertDoorstateIDToText(this.garageDoorLastCurrentDoorState), this.convertDoorstateIDToText(this.Characteristic.CurrentDoorState.OPENING), this.config.fakeGaragedoorParams.openTime);
    ...
    convertDoorstateIDToText(doorstate) {
        switch (doorstate) {
            case 0: return "OPEN";
